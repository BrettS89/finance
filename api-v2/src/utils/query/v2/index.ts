...
import { BaseQueryObject, ComparisonObject } from './schema';

export type GenerateQueryProps = {
  sqlString: string;
  filter: BaseQueryObject & { [field: string]: number | string | boolean | null | ComparisonObject & BaseQueryObject };
    ...
    if (field === '$or' && Array.isArray(value)) {
      value.forEach((el) => {
        ...
        if (orFilter.length > 1) {
          throw new BadRequestError('Invalid use of $or operator');
        }
